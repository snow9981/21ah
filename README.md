...
스물하나 동물병원의 이름과 철학을 빠르게 전달하고, 방문자가 네이버 플레이스, 블로그, 카카오톡 상담, 전화 문의로 이동하도록 만든 1페이지 정적 홈페이지입니다.
...
5. `index.html`을 새로고침해 변경 내용을 확인합니다.
...
`index.html`에서 placeholder 문자열을 검색한 뒤 실제 URL 또는 병원 정보로 바꾸면 됩니다.
...
`index.html`을 브라우저에서 바로 열어 확인할 수 있습니다. 정적 파일만 사용하므로 별도 서버 없이도 동작합니다.
